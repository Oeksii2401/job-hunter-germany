...
async def generate_search_queries_de(profile: dict, direction_text: str, lang: str) -> list:
    ...
    prompt = f"""Кандидат уточнил направление поиска работы в Германии: "{direction_text}"

(Справочно, ранее определённая сфера в профиле: "{primary_domain}" — используй её ТОЛЬКО если
она явно согласуется с новым направлением. Если направление кандидата описывает ДРУГУЮ,
не связанную область — полностью игнорируй primary_domain и строй запросы ИСКЛЮЧИТЕЛЬНО
на основе направления кандидата, не смешивая два разных запроса в одну фразу.)
